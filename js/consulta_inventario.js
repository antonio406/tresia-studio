...
function consulta_inventario(){
    document.getElementById("img_cargando").style.visibility = "visible";
        ...
        xhr.open('GET', `php/consulta_inventario.php?limit=${limit}&offset=${offset}`, true);
        xhr.onload = function() {
            if (xhr.status === 200) {
                ...
                clientas.forEach((clienta, index) => {
                    output += `
                        <tr>
                            <td>${startIndex + index}</td>
                                ...
                                <td>${clienta.unidad}</td>
                                <td>$${clienta.precio}</td>
                                <td align="center">
                            <button onclick="editar(${clienta.idproducto});" 
                            style="
                            ...
                            <td align="center">
                             <button onclick="eliminar(${clienta.idproducto});" 
                            style="
                    ...
                    document.getElementById("img_cargando").style.visibility = "hidden";
                    clientasList.innerHTML = '<tr><td colspan="10">No se encontraron Productos.</td></tr>';
  ...
  function eliminar(id) {
      Swal.fire({
          ...
          if (result.isConfirmed) {
              ...
              const xhr = new XMLHttpRequest();
              xhr.open('POST', 'php/elimina__producto.php', true);
                  ...
                  if (xhr.status === 200) {
                      ...
                      if (response.success) {
                          Swal.fire({
                            ...
                            consulta_inventario(); // Actualizar la lista de citas después de la eliminación
                          });
                      } else {
                          Swal.fire({
                              icon: 'error',
