    ...
    xhr.onload = function () {
        if (xhr.status === 200) {
            ...
            if (response.success) {
                ...
                clientas.forEach((clienta, index) => {
                    output += `
                            <tr>
                                ...
                                <td>$${clienta.monto}</td>
                                <td>$${clienta.monto_transferencia}</td>
                                <td>$${clienta.monto_efectivo}</td>
                                <td>${clienta.tipo}</td>
                                 <td align="center">
                ...
                document.getElementById("a").innerText = `Total de Anticipos: $${response.anticipo}`;
                ...
                document.getElementById("img_cargando").style.visibility = "hidden";
                document.getElementById("t").innerText = `Total Ingresos: $${response.ingreso || 0}`;
                document.getElementById("e").innerText = `Total de Gastos: $${response.gasto || 0}`;
                document.getElementById("a").innerText = `Total de Anticipos: $${response.anticipo || 0}`;
...
function editar(id) {
    if (!puedeEditar) {
        Swal.fire({
            ...
            text: 'No tienes permisos para editar gastos.',
            confirmButtonColor: '#d63384'
        });
        return;
    }
...
function eliminar(id) {
    if (!puedeEliminar) {
        Swal.fire({
            icon: 'error',
            title: 'Acceso denegado',
            text: 'No tienes permisos para eliminar gastos.',
            confirmButtonColor: '#d63384'
        });
        return;
    }
